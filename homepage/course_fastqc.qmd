...
We want to use FastQC on the samples, which is in the Pixi environment. So we preceed the `srun` command with `pixi run`, and add the FastQC specific commands:

```{.bash}
pixi run srun -A project_ID -t 15:00 -n 1 fastqc --noextract -o fastqc data data/sample_1.fastq.gz data/sample_2.fastq.gz 
```
...
Save and run with Pixi: 

```{.bash}
pixi run sbatch fastqc.sh 
```
